...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # 我改的，为了在twitter2015上训练，在twitter2017上测试
        eval_examples = processor.get_test_examples(tempTaskName)
        logger.info("  test dataset = %s", tempTaskName)
        eval_features = convert_mm_examples_to_features(
            eval_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.crop_size, args.path_image)
        ...
        test_data, imgs, _ = processor._read_mmtsv(os.path.join(tempTaskName, "test.txt"))
